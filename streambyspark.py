...
parser.add_argument('-t2', '--topic2', type=str, default="twitto1", help="The name of the topic. Carefull, this should be the same in producer.py")
...
df = spark \
    .readStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers",f"{args.sparkhost}:{args.sparkport},{args.sparkhost}:{args.sparkport}") \
    .option("subscribe", f"{args.topic},{args.topic2}") \
    .load()
...
#order4 = order3.filter(col("entities").contains(f"{args.fields}"))
...
''' query=(spark.readStream.format('mongodb').option("spark.mongodb.connection.uri", "mongodb+srv://moons:mongomoons@cluster0.eky2uou.mongodb.net/?retryWrites=true&w=majority") \
    	.option("spark.mongodb.database", "twitto") \
